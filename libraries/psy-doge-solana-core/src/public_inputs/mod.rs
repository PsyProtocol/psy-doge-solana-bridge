...
/// Computes public inputs for the custodian transition ZK proof.
///
...
/// have been consolidated using the next_index approach.
...
pub fn get_block_transition_public_inputs(
    ...
    let transition_hash =
        hash_impl_sha256_two_to_one_bytes(&previous_header_hash, &new_header_hash);
    hashv_impl_sha256_bytes(&[
        &transition_hash,
        config_params_hash,
        custodian_wallet_config_hash,
...
pub fn compute_backlog_hash(backlog_txo_mints: &[&FinalizedBlockMintTxoInfo]) -> QHash256 {
...
pub fn get_reorg_block_transition_public_inputs(
    ...
    let transition_and_backlog_hash =
        hash_impl_sha256_two_to_one_bytes(&transition_hash, &backlog_hash);
...
pub fn get_manual_deposit_proof_public_inputs(
    ...
    let recent_info = hash_impl_sha256_two_to_one_bytes(
        recent_block_merkle_tree_root,
        recent_auto_claim_txo_root,
    );
    let manual_claim_deposit_txo_transition = hash_impl_sha256_two_to_one_bytes(
        old_manual_claim_deposit_txo_root,
        new_manual_claim_txo_root,
    );
    let recent_info_with_user_txo_root =
        hash_impl_sha256_two_to_one_bytes(&recent_info, &manual_claim_deposit_txo_transition);
    ...
    hashv_impl_sha256_bytes(&[
        &recent_info_with_user_txo_root,
        &tx_info,
        custodian_wallet_config_hash,
    ])
}
