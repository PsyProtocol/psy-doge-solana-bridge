...
#[cfg(test)]
mod tests {
    use crate::crypto::hash::merkle::test_merkle_tree::SimpleMemoryMerkleRecorderStore;
    ...
    #[test]
    fn test_tree_append(){
        let mut tree = SimpleMemoryMerkleRecorderStore::<QSha256Hasher, QHash256>::new(32);
        let mut fixed_append = FixedMerkleAppendTree::new_empty();
        for i in 0..100 {
            let hash = rand_hash();
            let dmp_a = tree.set_leaf(i, hash);
            let dmp_b = fixed_append.append_delta_merkle_proof(hash);
            assert_eq!(dmp_a.index, dmp_b.index);
            assert_eq!(dmp_a.new_value, dmp_b.new_value);
            assert_eq!(dmp_a.old_value, dmp_b.old_value);
            assert_eq!(dmp_a.siblings, dmp_b.siblings);
            assert_eq!(dmp_a.old_root, dmp_b.old_root);
            assert_eq!(dmp_a.new_root, dmp_b.new_root);
            assert_eq!(dmp_a.new_root, tree.get_root());
            assert_eq!(fixed_append.get_root(), tree.get_root());
        }

    }

    #[test]
    fn test_tree_append_single() {
        let mut fixed_append = FixedMerkleAppendTree::new_empty();
        println!("Initial Root: {}", hex::encode(&fixed_append.current_root));
        let hash = hex_literal::hex!("00a3e11100000000000000001cda83b3928e6cb586f4b998237175aff2e572d3");
        fixed_append.append(hash);
        let mut tree = SimpleMemoryMerkleRecorderStore::<QSha256Hasher, QHash256>::new(32);
        tree.set_leaf(0, hash);
        assert_eq!(fixed_append.get_root(), tree.get_root());
        println!("Root: {}", hex::encode(&fixed_append.current_root));
